...
class GHAapp < Sinatra::Application
  ...
  before '/event_handler' do
    logger.debug 'about to get_payload_request'
    get_payload_request(request)
    logger.debug 'about to verify_webhook_signature'
    verify_webhook_signature
    logger.debug 'about to authenticate_app'
    authenticate_app
    logger.debug 'about to authenticate_installation'
  ...
  helpers do
    ...
    # Invoked whenever an issue is opened
    def handle_issue_opened_event(payload)
      repo = payload['repository']['full_name']
      issue_number = payload['issue']['number']
      logger.debug "An issue was opened from #{repo} with ##{issue_number}"
      @installation_client.add_labels_to_an_issue(repo, issue_number, ['bug'])
    end
